    ...
    async def _handle_command(self, user_input: str) -> bool:
        ...
        if user_input.startswith("/"):
            ...
            if len(parts) == 2:
                cmd_name, arg = parts
                if cmd_name == "connect":
                    await self._connect_provider_with_arg(arg)
                    return True
                elif cmd_name == "model":
...
"""
    ...
    async def _connect_provider(self) -> None:
        """Handle /connect command - show provider connection dialog."""
        from dotsy.core.providers.manager import get_provider_manager
        
        try:
            provider_mgr = await get_provider_manager(self.agent_loop.config)
            providers = provider_mgr.list_supported_providers()
            
            # Format provider list
            lines = ["## Connect to LLM Provider\n"]
            lines.append("**Supported Providers (75+ models):**\n")
            
            for provider in providers:
                status = "✓" if provider["id"] in provider_mgr.connections else "○"
                key_status = "🔑" if provider["requires_key"] else "🆓"
                local_badge = "🏠" if provider["is_local"] else "☁️"
                models_badge = "75+ models" if provider.get("supports_75_plus") else ""
                
                lines.append(
                    f"- `{status} /connect {provider['id']}` {local_badge} {key_status} "
                    f"**{provider['name']}** {models_badge}"
                )
            ...
            lines.extend([
                ...
                "/connect google        # Connect to Google",
                "/connect anthropic     # Connect to Anthropic",
                "/connect modelsdev     # Connect to Models.dev (75+ providers)",
                "/connect ollama        # Connect to local Ollama",
                "/connect lmstudio      # Connect to LM Studio",
                "```",
                "",
                "**Note:** For cloud providers, set API key in environment or `.env` file.",
    ...
    async def _select_model(self) -> None:
        """Handle /model command - show model selection or change active model."""
        from dotsy.core.providers.manager import get_provider_manager
        
        try:
            provider_mgr = await get_provider_manager(self.agent_loop.config)
            all_models = provider_mgr.list_all_models()
            
            if not all_models:
                lines = [
                    "## No Models Available\n",
                    "Connect to a provider first:",
                    "```",
                    "/connect openai",
                    "/connect modelsdev  # 75+ models",
            ...
            for model in all_models:
                provider = model.get("provider_name", model.get("provider", "unknown"))
            ...
            for provider_name, models in sorted(models_by_provider.items()):
                ...
                for model in models[:10]:  # Show first 10 per provider
                    full_id = model.get("full_id", model.get("id", ""))
                    is_free = "🆓" if model.get("is_free") else ""
                    context = model.get("context", 0)
                    context_str = f" ({context:,} ctx)" if context else ""
                    
                    lines.append(f"- `/model {full_id}` {is_free}{context_str}")
            ...
            lines.extend([
                ...
                "```",
                "/model openai/gpt-4o",
                "/model modelsdev/gpt-5",
                "/model modelsdev/claude-sonnet-4",
                "/model ollama/llama2",
                "```",
            ])
    ...
    async def _connect_provider_with_arg(self, provider_id: str) -> None:
        """Handle /connect <provider> - connect to specific provider."""
        from dotsy.core.providers.manager import get_provider_manager
        ...
        try:
            ...
            # Connect to provider
            conn = await provider_mgr.connect_provider(provider_id)
            ...
            await self._mount_and_scroll(
                UserCommandMessage(
                    ...
                    f"Available models: {len(conn.models)}\n\n"
                    f"Use `/model` to select a model."
                )
            )
            
        except ValueError as e:
            await self._mount_and_scroll(
                UserCommandMessage(f"❌ {str(e)}\n\nUse `/connect` to see available providers.")
            )
        except Exception as e:
            await self._mount_and_scroll(
                ErrorMessage(f"Failed to connect: {e}", collapsed=self._tools_collapsed)
            )

    async def _select_model_with_arg(self, model_id: str) -> None:
        """Handle /model <model_id> - select specific model."""
        from dotsy.core.providers.manager import get_provider_manager
        ...
        try:
            ...
            # Find the model
            model = provider_mgr.get_model(model_id)
            if not model:
                await self._mount_and_scroll(
                    UserCommandMessage(
            ...
            model_name = model.get("id", model_id)
            
            # Find or create model config
            existing_model = next(
                (m for m in self.agent_loop.config.models 
                 if m.name == model_name and m.provider == provider_name),
                None
            )
            
            if not existing_model:
                from dotsy.core.config import ModelConfig
                ...
                self.agent_loop.config.models.append(new_model)
            ...
            await self._mount_and_scroll(
                UserCommandMessage(
                    ...
                    f"Provider: {model.get('provider_name', provider_name)}\n"
                    f"Model: {model.get('name', model_name)}"
                )
            )
            
        except Exception as e:
            await self._mount_and_scroll(
                ErrorMessage(f"Failed to select model: {e}", collapsed=self._tools_collapsed)
            )
