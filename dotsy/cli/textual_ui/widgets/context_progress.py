...
class ContextProgress(NoMarkupStatic):
    ...
    def watch_tokens(self, new_state: TokenState) -> None:
        ...
        # Reset tracking if max_tokens changed (model switch)
        if new_state.max_tokens != self._last_max_tokens:
            self.tracking = TokenTracking(
                start_time=time.time(), last_tokens=new_state.current_tokens
            )
            self._last_max_tokens = new_state.max_tokens
        ...
        # Reset tracking if tokens decreased significantly (e.g., after compact)
