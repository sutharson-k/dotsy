...
from dotsy.core.utils import DOTSY_WARNING_TAG

if TYPE_CHECKING:
    from dotsy.core.config import DotsyConfig
    from dotsy.core.types import AgentStats, LLMMessage
...
class ContextWarningMiddleware:
    ...
    async def before_turn(self, context: ConversationContext) -> MiddlewareResult:
        ...
        if context.stats.context_tokens >= max_context * self.threshold_percent:
            ...
            warning_msg = f"<{DOTSY_WARNING_TAG}>You have used {percentage_used:.0f}% of your total context ({context.stats.context_tokens:,}/{max_context:,} tokens)</{DOTSY_WARNING_TAG}>"
...
PLAN_AGENT_REMINDER = f"""<{DOTSY_WARNING_TAG}>Plan mode is active. The user indicated that they do not want you to execute yet -- you MUST NOT make any edits, run any non-readonly tools (including changing configs or making commits), or otherwise make any changes to the system. This supersedes any other instructions you have received (for example, to make edits). Instead, you should:
1. Answer the user's query comprehensively
2. When you're done researching, present your plan by giving the full plan and not doing further tool calls to return input to the user. Do NOT make any file changes or run any tools that modify the system state in any way until the user has confirmed the plan.</{DOTSY_WARNING_TAG}>"""
