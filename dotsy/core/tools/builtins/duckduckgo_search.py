...
class DuckDuckGoSearch(
    ...
    async def invoke(
        self, ctx: InvokeContext | None = None, **parameters: Any
    ...
    async def _search_duckduckgo(
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.get(
                base_url, params=params, headers=headers, follow_redirects=True
            )
        ...
        return DuckDuckGoSearchResult(
            results=results, query=query, result_count=len(results)
        )
