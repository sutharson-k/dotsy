...
from dotsy.core.autocompletion.completers import PathCompleter


@pytest.fixture()
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    (tmp_path / "dotsy" / "acp").mkdir(parents=True)
    (tmp_path / "dotsy" / "acp" / "entrypoint.py").write_text("")
    (tmp_path / "dotsy" / "acp" / "agent.py").write_text("")
    (tmp_path / "dotsy" / "cli" / "autocompletion").mkdir(parents=True)
    (tmp_path / "dotsy" / "cli" / "autocompletion" / "fuzzy.py").write_text("")
    (tmp_path / "dotsy" / "cli" / "autocompletion" / "completers.py").write_text("")
...
def test_finds_files_recursively_by_filename(file_tree: Path) -> None:
    ...
    assert results[0] == "@dotsy/acp/entrypoint.py"
...
def test_finds_multiple_matches_recursively(file_tree: Path) -> None:
    ...
    dotsy_index = results.index("@dotsy/cli/autocompletion/fuzzy.py")
    test_index = results.index("@tests/autocompletion/test_fuzzy.py")
    assert dotsy_index < test_index


def test_prioritizes_exact_path_matches(file_tree: Path) -> None:
    results = PathCompleter().get_completions("@dotsy/acp/entrypoint", cursor_pos=20)
...
def test_finds_files_when_pattern_matches_directory_name(file_tree: Path) -> None:
    ...
    assert results == [
        ...
        "@dotsy/cli/autocompletion/completers.py",
        "@tests/autocompletion/",
        "@tests/autocompletion/test_fuzzy.py",
        "@dotsy/cli/autocompletion/",
        "@dotsy/cli/autocompletion/fuzzy.py",
    ]
